...
echo "# lrResponseVariables" >> README.md
git init
git add .
...
newQuestion =
    ...
    in
    Random.map
        -- Make a new variable of type "Question" with the stem and image from above and a
        -- list of possible responses for the user to choose from
        (Question newStem newImage)
        -- The "andThen" thing blew my mind for a while. Basically, if you have
        -- Random.andThen A B
        -- 1. B creates some random thing
        -- 2. A takes that random thing and repackages it into a new random thing
        -- First create a list of 3 randomly-chosen responses (1 correct, 2 distractors)
        (newListOfResponses 2 listOfRightAnswers listOfDistractors
            -- Then shuffle that list into a random order
